...
async def lookup_xauth(loop, auth_path, host, dpynum):
    ...
    for entry in walk_xauth(auth_path):
        ...
        if entry.family == XAUTH_FAMILY_IPV4:
            if not ipv4_addrs:
                ipv4_addrs = await _lookup_host(loop, host,
                                                     socket.AF_INET)
            ...
            if not ipv6_addrs:
                ipv6_addrs = await _lookup_host(loop, host,
                                                     socket.AF_INET6)
...
async def update_xauth(loop, auth_path, host, dpynum, auth_proto, auth_data):
    ...
    for _ in range(XAUTH_LOCK_TRIES):
        try:
            ...
            await asyncio.sleep(XAUTH_LOCK_DELAY, loop=loop)
        else:
            break
...
async def create_x11_client_listener(loop, display, auth_path):
    ...
    auth_proto, auth_data = await lookup_xauth(loop, auth_path,
                                                    host, dpynum)

    return SSHX11ClientListener(loop, host, dpynum, auth_proto, auth_data)


async def create_x11_server_listener(conn, loop, auth_path, auth_proto, auth_data):
    ...
    for dpynum in range(X11_DISPLAY_START, X11_MAX_DISPLAYS):
        ...
        try:
            await update_xauth(loop, auth_path, X11_LISTEN_HOST, dpynum,
                                    auth_proto, auth_data)
        except ValueError:
            tcp_listener.close()
            break
