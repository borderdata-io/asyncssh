...
class _SCPCopier:
    ...
    async def _copy_files(self):
        ...
        while True:
            ...
            exc = await self._forward_response(self._sink,
                                                    self._source)
...
async def scp(srcpaths, dstpath=None, *, preserve=False, recurse=False,
    ...
    try:
        for srcpath in srcpaths:
            srcconn, srcpath, close_src = await _parse_path(srcpath,
                                                                 **kwargs)
...
async def run_scp_server(sftp_server, command, stdin, stdout, stderr):
