...
class SSHConnection(SSHPacketHandler):
    ...
    async def forward_unix_connection(self, dest_path):
        ...
        try:
            _, peer = \
                await self._loop.create_unix_connection(SSHForwarder,
                                                             dest_path)
...
class SSHClientConnection(SSHConnection):
    ...
    async def create_session(self, session_factory, command=None, *, subsystem=None,
    ...
    async def create_connection(self, session_factory, remote_host, remote_port,
        ...
        session = await chan.connect(session_factory, remote_host,
    ...
    async def forward_socks(self, listen_host, listen_port):
        ...
        try:
            return (await create_socks_listener(self, self._loop,
...
class SSHServerConnection(SSHConnection):
    ...
    async def validate_public_key(self, username, key_data, msg, signature):
        ...
        key = ((await self._validate_client_certificate(username,
                                                             key_data)) or
               (await self._validate_client_public_key(username,
                                                            key_data)))
    ...
    async def create_agent_listener(self):
        ...
        if not self._agent_listener:
            self._agent_listener = await create_agent_listener(self,
                                                                    self._loop)
...
async def create_connection(client_factory, host, port=_DEFAULT_PORT, *,
    ...
    if client_keys:
        ...
        if agent_path:
            agent = await connect_agent(agent_path, loop=loop)
...
async def create_server(server_factory, host=None, port=_DEFAULT_PORT, *,
    ...
    return (await loop.create_server(conn_factory, host, port,
...
async def connect(host, port=_DEFAULT_PORT, **kwargs):
    ...
    conn, _ = await create_connection(None, host, port, **kwargs)

    return conn


async def listen(host=None, port=_DEFAULT_PORT, **kwargs):
    """Start an SSH server
    ...
    return (await create_server(None, host, port, **kwargs))
