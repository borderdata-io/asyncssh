    ...
    def sftp_wrapper(self):
        ...
        with (await self.connect()) as conn:
            with (await conn.start_sftp_client()) as sftp:
                await asyncio.coroutine(func)(self, sftp)
...
class _TestSFTP(_CheckSFTP):
    ...
    def test_glob(self, sftp):
        ...
        try:
            ...
            self.assertEqual(sorted((await sftp.glob('./file*'))),
                             ['./file1', './filedir'])
            self.assertEqual(sorted((await sftp.glob(b'file*'))),
                             [b'file1', b'filedir'])
            self.assertEqual(sorted((await sftp.glob(['file*']))),
                             ['file1', 'filedir'])
            self.assertEqual(sorted((await sftp.glob(['', 'file*']))),
                             ['file1', 'filedir'])
            self.assertEqual(sorted((await sftp.glob(['file*/*2']))),
                             ['filedir/file2'])
            self.assertEqual(sorted((await sftp.glob(['file*/*[3-9]']))),
                             ['filedir/file3'])
            self.assertEqual(sorted((await sftp.glob(['**/file[12]']))),
                             ['file1', 'filedir/file2'])
            self.assertEqual(sorted((await sftp.glob(['**/file*/']))),
                             ['filedir'])
    ...
    def test_symlink_encode_error(self):
        ...
        with (await self.connect()) as conn:
            sftp = await conn.start_sftp_client(path_encoding=None)
            with sftp:
                with self.assertRaises(SFTPError):
                    await sftp.symlink('file', 'link')
    ...
    def test_nonstandard_symlink_client(self):
        ...
        try:
            with (await self.connect(client_version='OpenSSH')) as conn:
                with (await conn.start_sftp_client()) as sftp:
                    await sftp.symlink('link', 'file')
                    self._check_link('link', 'file') # pragma: no branch

                await sftp.wait_closed() # pragma: no branch
    ...
    def test_open_read_out_of_order(self):
        ...
        async def _test_read_out_of_order(self, sftp):
            ...
            try:
                ...
                with (await sftp.open('file')) as f:
                    await f.read()
            finally:
    ...
    def test_exit_after_nonblocking_close(self, sftp):
        ...
        with (await sftp.open('nonblocking_file', 'w')):
            pass
    ...
    def test_write_close(self):
        ...
        async def _write_close(self, sftp):
            ...
            try:
                with (await sftp.open('file', 'w')) as f:
                    with self.assertRaises(SFTPError):
                        await f.write('a')
            finally:
                sftp.exit()
