...
class _TestTCPForwarding(_CheckForwarding):
    ...
    def test_ssh_connect_tunnel(self):
        ...
        with (await self.connect()) as conn:
            with (await conn.connect_ssh(self._server_addr,
    ...
    def test_forward_bind_error_ipv4(self):
        ...
        with (await self.connect()) as conn:
            listener = await conn.forward_local_port('0.0.0.0', 0,
                                                          '', 7)
    ...
    def test_forward_connect_error(self):
        ...
        with (await self.connect()) as conn:
            listener = await conn.forward_local_port('', 0, '', 0)
            listen_port = listener.get_port()
    ...
    def test_forward_immediate_eof(self):
        ...
        with (await self.connect()) as conn:
            ...
            _, writer = await asyncio.open_connection(None,
                                                           listen_port)
    ...
    def test_forward_remote_port(self):
        ...
        with (await self.connect()) as conn:
            listener = await conn.forward_remote_port('', 0, '',
                                                           server_port)
...
class _TestUNIXForwarding(_CheckForwarding):
    ...
    def test_unix_server_open(self):
        ...
        with (await self.connect()) as conn:
            ...
            listener = await conn.start_unix_server(handler_factory,
                                                         'open')
    ...
    def test_unix_server_failure(self):
        ...
        with (await self.connect()) as conn:
            listener = await conn.start_unix_server(_unix_listener,
                                                         'fail')
            self.assertIsNone(listener)
...
class _TestSOCKSForwarding(_CheckForwarding):
    ...
    async def _check_socks(self, handler, listen_port, msg, data, *args):
        ...
        with self.subTest(msg=msg, data=data):
            ...
            reader, writer = \
                await asyncio.open_connection(None, listen_port)
